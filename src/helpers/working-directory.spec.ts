...
describe('applyWorkingDirectory', () => {
  ...
  beforeEach(() => {
    chdir = jest.spyOn(process, 'chdir').mockImplementation(() => {
      // stubbed; process.cwd() is mocked separately per test
    })
  ...
  it('changes into a subdirectory and reports its path relative to the repo root', () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith('git', ['rev-parse', '--show-toplevel'], {
      encoding: 'utf8',
    })
  ...
  it('reports an empty prefix when the working directory is itself the repo root', () => {
    ...
    mockedExecFileSync.mockReturnValue('/nested/checkout\n')
